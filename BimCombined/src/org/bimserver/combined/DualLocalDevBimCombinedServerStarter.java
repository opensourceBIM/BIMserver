...
public class DualLocalDevBimCombinedServerStarter {
	private org.eclipse.jetty.server.Server server;
	private static final Logger LOGGER = LoggerFactory.getLogger(DualLocalDevBimCombinedServerStarter.class);

	public static void main(String[] args) {
		new DualLocalDevBimCombinedServerStarter().start("localhost", 8080, 8081);
	}
	...
	public void start(String address, int... ports) {
		for (final int port : ports) {
			Thread thread = new Thread(new Runnable(){
				...
				public void run() {
					...
					config.setHomeDir(new File("home" + port));
					config.setResourceFetcher(new LocalDevelopmentResourceFetcher());
					config.setStartEmbeddedWebServer(true);
					config.setClassPath(System.getProperty("java.class.path"));
					config.setPort(port);
					config.setInitialProtocolBuffersPort(port + 20);
