...
public class ComplexLine implements Comparable<ComplexLine> {
	...
	public ComplexLine(int index1, int index2, DoubleBuffer verticesAsDouble, FloatBuffer normalsAsFloat, float margin) {
		if (index1 > index2) {
			this.index1 = index2;
			this.index2 = index1;
		} else {
		...
		this.normal = new double[] {round(normalsAsFloat.get(index1 * 3), margin), round(normalsAsFloat.get(index1 * 3 + 1), margin), round(normalsAsFloat.get(index1 * 3 + 2), margin)};
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		ComplexLine other = (ComplexLine) obj;
	...
	public String toString() {
		return Arrays.toString(v1) + " - "  + Arrays.toString(v2) + " - " + Arrays.toString(normal);
	}

	@Override
	public int compareTo(ComplexLine o) {
		for (int i=0; i<3; i++) {
			if (this.v1[i] == o.v1[i]) {
				// Continue
			} else {
		...
		for (int i=0; i<3; i++) {
			if (this.normal[i] == o.normal[i]) {
				// Continue
			} else {
				return Double.compare(this.normal[i], o.normal[i]);
			}
		}
		return 0;
	}
