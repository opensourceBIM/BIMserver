...
public class SConverter {
	...
	public Set<SGeometryInstance> convertToSSetGeometryInstance(Collection<GeometryInstance> input) {
		Set<SGeometryInstance> result = new HashSet<SGeometryInstance>();
		for (GeometryInstance o : input) {
			result.add(convertToSObject(o));
		}
		return result;
	}

	public Set<GeometryInstance> convertFromSSetGeometryInstance(Collection<SGeometryInstance> input, DatabaseSession session) throws BimserverDatabaseException {
		Set<GeometryInstance> result = new HashSet<GeometryInstance>();
		for (SGeometryInstance o : input) {
			result.add(convertFromSObject(o, session));
		}
		return result;
	}

	public List<SGeometryInstance> convertToSListGeometryInstance(Collection<GeometryInstance> input) {
		List<SGeometryInstance> result = new ArrayList<SGeometryInstance>();
		for (GeometryInstance o : input) {
			result.add(convertToSObject(o));
		}
		return result;
	...
	public List<GeometryInstance> convertFromSListGeometryInstance(Collection<SGeometryInstance> input, DatabaseSession session) throws BimserverDatabaseException {
		List<GeometryInstance> result = new ArrayList<GeometryInstance>();
		for (SGeometryInstance o : input) {
			result.add(convertFromSObject(o, session));
		}
		return result;
	...
	public SGeometryInstance convertToSObject(GeometryInstance input) {
		if (input == null) {
			return null;
		}
		
		SGeometryInstance result = new SGeometryInstance();
		result.setOid(input.getOid());
		result.setRid(input.getRid());
	...
	public GeometryInstance convertFromSObject(SGeometryInstance input, DatabaseSession session) throws BimserverDatabaseException {
		if (input == null) {
			return null;
		}		
		GeometryInstance result = StoreFactory.eINSTANCE.createGeometryInstance();
		((IdEObjectImpl)result).setOid(input.getOid());
		((IdEObjectImpl)result).setRid(input.getRid());
		result.setStartVertex(input.getStartVertex());
		result.setStartIndex(input.getStartIndex());
