...
public class SConverter {
	...
	public SUserSettings convertToSObject(UserSettings input) {
		...
		result.setServices(listservices);
		List<Long> listmodelCheckers = new ArrayList<Long>();
		for (ModelCheckerPluginConfiguration v : input.getModelCheckers()) {
			listmodelCheckers.add(v.getOid());
		}
		result.setModelCheckers(listmodelCheckers);
		return result;
	...
	public UserSettings convertFromSObject(SUserSettings input, UserSettings result, DatabaseSession session) throws BimserverDatabaseException {
		...
		List<ModelCheckerPluginConfiguration> listmodelCheckers = result.getModelCheckers();
		for (long oid : input.getModelCheckers()) {
			listmodelCheckers.add((ModelCheckerPluginConfiguration)session.get(StorePackage.eINSTANCE.getModelCheckerPluginConfiguration(), oid, Query.getDefault()));
		}
		return result;
	}
	...
	public SPluginConfiguration convertToSObject(PluginConfiguration input) {
		...
		else if (input instanceof ModelCheckerPluginConfiguration) {
			return convertToSObject((ModelCheckerPluginConfiguration)input);
		}
	...
	public PluginConfiguration convertFromSObject(SPluginConfiguration input, PluginConfiguration result, DatabaseSession session) throws BimserverDatabaseException {
		...
		else if (input instanceof SModelCheckerPluginConfiguration) {
			return convertFromSObject((SModelCheckerPluginConfiguration)input, session);
		}
	...
	public PluginDescriptor convertFromSObject(SPluginDescriptor input, PluginDescriptor result, DatabaseSession session) throws BimserverDatabaseException {
		...
		if (input instanceof SModelComparePluginDescriptor) {
	...
	public STypeDefinition convertToSObject(TypeDefinition input) {
		...
		if (input instanceof PrimitiveDefinition) {
	...
	public TypeDefinition convertFromSObject(STypeDefinition input, TypeDefinition result, DatabaseSession session) throws BimserverDatabaseException {
		...
		if (input instanceof SPrimitiveDefinition) {
	...
	public Set<SModelCheckerPluginConfiguration> convertToSSetModelCheckerPluginConfiguration(Collection<ModelCheckerPluginConfiguration> input) {
		Set<SModelCheckerPluginConfiguration> result = new HashSet<SModelCheckerPluginConfiguration>();
		for (ModelCheckerPluginConfiguration o : input) {
			result.add(convertToSObject(o));
		}
		return result;
	}

	public Set<ModelCheckerPluginConfiguration> convertFromSSetModelCheckerPluginConfiguration(Collection<SModelCheckerPluginConfiguration> input, DatabaseSession session) throws BimserverDatabaseException {
		Set<ModelCheckerPluginConfiguration> result = new HashSet<ModelCheckerPluginConfiguration>();
		for (SModelCheckerPluginConfiguration o : input) {
			result.add(convertFromSObject(o, session));
		}
		return result;
	}

	public List<SModelCheckerPluginConfiguration> convertToSListModelCheckerPluginConfiguration(Collection<ModelCheckerPluginConfiguration> input) {
		List<SModelCheckerPluginConfiguration> result = new ArrayList<SModelCheckerPluginConfiguration>();
		for (ModelCheckerPluginConfiguration o : input) {
			result.add(convertToSObject(o));
		}
		return result;
	...
	public List<ModelCheckerPluginConfiguration> convertFromSListModelCheckerPluginConfiguration(Collection<SModelCheckerPluginConfiguration> input, DatabaseSession session) throws BimserverDatabaseException {
		List<ModelCheckerPluginConfiguration> result = new ArrayList<ModelCheckerPluginConfiguration>();
		for (SModelCheckerPluginConfiguration o : input) {
			result.add(convertFromSObject(o, session));
		}
		return result;
	...
	public SModelCheckerPluginConfiguration convertToSObject(ModelCheckerPluginConfiguration input) {
		if (input == null) {
			return null;
		}
		
		SModelCheckerPluginConfiguration result = new SModelCheckerPluginConfiguration();
		result.setOid(input.getOid());
		result.setRid(input.getRid());
		result.setName(input.getName());
		result.setEnabled(input.getEnabled());
	...
	public ModelCheckerPluginConfiguration convertFromSObject(SModelCheckerPluginConfiguration input, DatabaseSession session) throws BimserverDatabaseException {
		ModelCheckerPluginConfiguration result = convertFromSObject(input, StoreFactory.eINSTANCE.createModelCheckerPluginConfiguration(), session);
	...
	public ModelCheckerPluginConfiguration convertFromSObject(SModelCheckerPluginConfiguration input, ModelCheckerPluginConfiguration result, DatabaseSession session) throws BimserverDatabaseException {
		if (input == null) {
			return null;
		}
		result.setName(input.getName());
	...
	public SModelCheckerInstance convertToSObject(ModelCheckerInstance input) {
		...
		ModelCheckerPluginConfiguration modelCheckerVal = input.getModelChecker();
		result.setModelCheckerId(modelCheckerVal == null ? -1 : modelCheckerVal.getOid());
		return result;
	}
	...
	public ModelCheckerInstance convertFromSObject(SModelCheckerInstance input, ModelCheckerInstance result, DatabaseSession session) throws BimserverDatabaseException {
		...
		result.setCode(input.getCode());
		result.setModelChecker((ModelCheckerPluginConfiguration)session.get(StorePackage.eINSTANCE.getModelCheckerPluginConfiguration(), input.getModelCheckerId(), Query.getDefault()));
		return result;
	}
	...
	public UserRelated convertFromSObject(SUserRelated input, UserRelated result, DatabaseSession session) throws BimserverDatabaseException {
		...
		if (input instanceof SUserChanged) {
