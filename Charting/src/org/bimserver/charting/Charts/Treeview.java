...
public class Treeview extends Chart {
	...
	public Treeview(String title) {
		this(
			title,
			"The tree layout implements the Buchheim-Juenger-Sebastian algorithm (found in \"Improving Walker's Algorithm to Run in Linear Time\") as an implementation of Walker's algorithm, which is a generalization of Reingold-Tilford algorithm, which is designed for efficient, tidy arrangement of layered nodes. The depth of nodes is computed by distance from the root, leading to a ragged appearance. <br/>Based on <a href='http://bl.ocks.org/mbostock/4339184'>http://bl.ocks.org/mbostock/4339184</a>",
			"Hierarchies",
	 ...
	 * @param fitToWidth
	 */
	public Treeview(String title, String description, String category, ArrayList<ChartOption> options, org.bimserver.charting.Models.Model model, boolean fitToWidth) {
	...
	public StringBuilder writeSVGChartSpecificPayload(StringBuilder builder, ChartRows filteredData) {
		...
		double effectiveBlankSpaceHeight = height - leafCount * 11;
		...
		NodeLinkTreeLayout layout = new NodeLinkTreeLayout("tree", Constants.ORIENT_LEFT_RIGHT, width / depth, 11, effectiveBlankSpaceHeight / leafCount);
