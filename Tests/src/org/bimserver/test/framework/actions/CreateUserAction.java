...
public class CreateUserAction extends Action {
	...
	public void execute(VirtualUser virtualUser) throws ServerException, UserException, PublicInterfaceNotFoundException {
		...
		SUser user = virtualUser.getBimServerClient().getService().addUser(username, randomString(), SUserType.values()[nextInt(SUserType.values().length)], nextBoolean());
		virtualUser.getBimServerClient().getAuth().changePassword(user.getOid(), "", "test");
